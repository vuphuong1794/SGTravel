...
<head>
    <meta charset="UTF-8">
    <title>Bình luận tháng này</title>
...
<body>
    ...
    $countQuery = "SELECT COUNT(*) AS total FROM binh_luan WHERE MONTH(ngay_binh_luan) = MONTH(CURDATE()) 
    AND YEAR(ngay_binh_luan) = YEAR(CURDATE())";
    ...
    <div class="container">
        <h2 align="center">Bình luận tháng này</h2>
                ...
                if ($Loc_Comments && $Loc_Comments->num_rows > 0) {
                    while ($comment = $Loc_Comments->fetch_assoc()) { ?>
                        <tr>
                            ...
                            <td>
                                <a href="binhluanthangnay.php?delete_id=<?php echo $comment['id']; ?>" onclick="return confirm('Bạn có chắc muốn xóa bình luận này?');">
                                    <button>Xóa</button>
                                </a>
                            </td>
                        </tr>
        ...
        <div class="trang">
            <?php if ($page > 1) { ?>
                <a href="binhluanthangnay.php?page=<?php echo $page - 1; ?>&id_dia_diem=<?php echo $id_DD; ?>">Trang trước</a>
            <?php } ?>
            
            <?php for ($i = 1; $i <= $tst; $i++) { ?>
                <a href="binhluanthangnay.php?page=<?php echo $i; ?>&id_dia_diem=<?php echo $id_DD; ?>" <?php if ($page == $i) echo 'class="active"'; ?>>
                    <?php echo $i; ?>
                </a>
            <?php } ?>
            
            <?php if ($page < $tst) { ?>
                <a href="binhluanthangnay.php?page=<?php echo $page + 1; ?>&id_dia_diem=<?php echo $id_DD; ?>">Trang sau</a>
            <?php } ?>
        </div>
        </p>
    </div>

...
<head>
  ...
  <title>Bài đăng tháng này</title>
...
<body>
  ...
  <div class="container">
   ...
   // query bài đăng tháng này
   $sql = "SELECT * from dia_diem
        WHERE MONTH(ngay_dang) = MONTH(CURDATE()) 
        AND YEAR(ngay_dang) = YEAR(CURDATE()) $keywordCondition";
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        echo "</td>
                      ...
                      <td>
                             <button onclick=\"if(confirm('Bạn có chắc chắn muốn xóa địa điểm này?')) { window.location.href='xoa_dia_diem_thang.php?id=" . urlencode($row["id"]) . "'; }\">Xóa địa điểm</button>
                      </td>
                    </tr>";
      }
      echo "</table>";

...
<body>
    ...
    // Lọc bình luận theo địa điểm nếu id_dia_diem được chọn và ngày hiện tại
    ...
    $Loc_Query = "SELECT binh_luan.id, binh_luan.noi_dung, binh_luan.id_dia_diem, 
                    ...
                    WHERE DATE(binh_luan.ngay_binh_luan) = '$today'";

    // Nếu có địa điểm được chọn, thêm điều kiện lọc địa điểm
    if ($id_DD > 0) {
        $Loc_Query .= " AND binh_luan.id_dia_diem = $id_DD";
    }
    ...
    $countQuery = "SELECT COUNT(*) AS total FROM binh_luan WHERE DATE(ngay_binh_luan) = '$today'";
    if ($id_DD > 0) {
        $countQuery .= " AND id_dia_diem = $id_DD";
    }
    ...
    <div class="container">
        <h2 align="center">Bình luận hôm nay</h2>
                ...
                if ($Loc_Comments && $Loc_Comments->num_rows > 0) {
                    while ($comment = $Loc_Comments->fetch_assoc()) { ?>
                        <tr>
                            ...
                            <td>
                                <a href="binhluanhomnay.php?delete_id=<?php echo $comment['id']; ?>" onclick="return confirm('Bạn có chắc muốn xóa bình luận này?');">
                                    <button>Xóa</button>
                                </a>
                            </td>
                        </tr>
        ...
        <div class="trang">
            <?php if ($page > 1) { ?>
                <a href="binhluanhomnay.php?page=<?php echo $page - 1; ?>&id_dia_diem=<?php echo $id_DD; ?>">Trang trước</a>
            <?php } ?>
            
            <?php for ($i = 1; $i <= $tst; $i++) { ?>
                <a href="binhluanhomnay.php?page=<?php echo $i; ?>&id_dia_diem=<?php echo $id_DD; ?>" <?php if ($page == $i) echo 'class="active"'; ?>>
                    <?php echo $i; ?>
                </a>
            <?php } ?>
            
            <?php if ($page < $tst) { ?>
                <a href="binhluanhomnay.php?page=<?php echo $page + 1; ?>&id_dia_diem=<?php echo $id_DD; ?>">Trang sau</a>
            <?php } ?>
        </div>
        </p>
    </div>

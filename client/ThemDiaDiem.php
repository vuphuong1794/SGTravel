...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Vòng lặp để xử lý upload ảnh
    for ($i = 1; $i <= $image_count; $i++) {
        if (isset($_FILES["hinh_anh$i"]) && $_FILES["hinh_anh$i"]['name'] != '') {
            $hinh_anh = mysqli_real_escape_string($conn, $_FILES["hinh_anh$i"]['name']);
            move_uploaded_file($_FILES["hinh_anh$i"]['tmp_name'], "../images/locations/" . $hinh_anh);

            $image_columns .= ", hinh_anh$i";
            $image_values .= ", '$hinh_anh'";
        } else {
            // Xử lý trường hợp ảnh không được gửi
            $image_columns .= ", hinh_anh$i";
            $image_values .= ", NULL";
        }
    }
